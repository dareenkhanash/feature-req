...
feature_req=Blueprint('feature_req',__name__, static_folder = './feature_req/static')   
...
def get_features():
    try:
        ...
        if requests:
            ...
            return jsonify({'requests':output}),201
        else:
            return jsonify({'messeage':'no requests'}),200
...
def add_request():
    try:
        if request.method == 'POST':
            ...
            print(request.form)
            return jsonify({'messeage':'request added susccesfully'}),201
    ...
    try:
        if request.method == 'DELETE':
            if request_id:
               ...
               return jsonify({"message": "Request deleted"}),201
            else:
               return jsonify({"error": "No Request Id"}),404
    except Exception as e:
        return jsonify({"error":"error: {0}".format(e)})
...
def update_request():
    try:
        ...
        #passes request information from form
           ...
           return jsonify({"message": "request updated"}),201
    except Exception as e:
        return jsonify({"error":"error: {0}".format(e)})
